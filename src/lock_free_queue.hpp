...
template<typename T>
void LockFreeQueue<T>::set_new_tail(CountedNodePtr& old_tail, CountedNodePtr const &new_tail) {
...
void LockFreeQueue<T>::push(T new_value) {
    ...
    for(;;) {
        ...
        if (dummy_node->data.compare_exchange_strong(old_data, new_data.get())) {
            // 线程A成功
            // If the call succeeds, this is your tail node, and you can safely set the
